...
const VideoChatApp = () => {
  ...
  console.log(client.connectionState, localTracks);
  ...
  const handleUserPublished = async (user, mediaType) => {
    try {
      ...
      if (mediaType === "video") {
        ...
        remoteVideoContainer.className = "remote-video"; // Add a class for styling
  ...
  useEffect(() => {
    if (screenShareTrack) {
      ...
      screenShareTrack.on("track-ended", () => {
        console.log("Screen sharing stopped via default button");
        stopScreenShare();
      });
  ...
  const notifyUsersAdminLeft = () => {
    socket.emit("adminLeft", roomId);
  };

  // Listen for admin left notifications
  ...
  return (
    <main
      className="w-full flex flex-col bg-slate-800 h-screen  justify-between"
      ref={fullscreenRef}
    ...
    >
      ...
      <div className="w-full sm:h-[70%] h-[80%] lg:h-[80%] flex   mt-2">
        ...
        >
          <div className=" sm:h-full lg:w-full items-center flex flex-col">
